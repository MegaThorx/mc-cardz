...
export default ({question, name, title, answer, setAnswer}: AnswerInputProps) => {
    ...
    return <div className="mb-3">
        ...
        <div className="form-check mt-2">
            <input className="form-check-input" type="checkbox" checked={answer.isCorrect} id={`${name}-correct`}
                   onChange={(event) => {
                       setAnswer({
                           id: answer.id,
                           text: answer.text,
        ...
        <div className="form-check mt-2">
            <input className="form-check-input" type="checkbox" checked={answer.isAiGenerated} id={`${name}-ai`}
                   onChange={(event) => {
                       setAnswer({
                           id: answer.id,
                           text: answer.text,
